...
| Packaged transcription recovery | A long-lived desktop server can retain a removed app-bundle working directory; whisper.cpp then aborts during backend discovery before reading audio | Run FFmpeg, FFprobe, and whisper.cpp from the stable private work directory; retry one native crash once and log only exit classification | Persist stage checkpoints and recover interrupted analysis after an app restart | `src/lib/server/local-analysis.ts`, `src/lib/ai/local-process-recovery.ts`, desktop build verification | 1 | High |
